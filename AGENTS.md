...
that opens the gate on its own: `Dashboard.CurrentScreen` reports when it
advanced and `App.nextCycle` ORs that into *due*, so a rotation reaches the
panel on the cycle it happens rather than waiting for the new screen's next
due widget. Don't confuse a widget's top-level
