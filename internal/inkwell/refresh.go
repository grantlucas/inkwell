...
// changes (plus a periodic refresh on the same cadence to guard against burn-in).
...
//
// The planner only chooses the waveform. App.refresh runs the complete
// hardware lifecycle around every non-skip decision, in either color mode: a
// hardware reset and the mode's init sequence, the frame push, and the sleep
// sequence that powers the panel off until the next push. There is therefore
// no separate "re-init needed" signal to plan — every push re-inits.
