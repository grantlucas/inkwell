...
func TestFilterEventsForDay(t *testing.T) {
	...
	day := time.Date(2026, 3, 19, 0, 0, 0, 0, toronto)
	dayEnd := day.AddDate(0, 0, 1)

	// An iCal VALUE=DATE is anchored to UTC midnight, but the column is
	// built in the viewer's zone. Compared as instants, a Thursday
	// all-day event lands on Wednesday in any negative-UTC zone.
	...
	timed := calendar.Event{
		Summary: "Standup",
		Start:   time.Date(2026, 3, 19, 13, 0, 0, 0, time.UTC),
		End:     time.Date(2026, 3, 19, 14, 0, 0, 0, time.UTC),
	}
	otherDay := calendar.Event{
		Summary: "Next week",
		Start:   time.Date(2026, 3, 26, 13, 0, 0, 0, time.UTC),
		End:     time.Date(2026, 3, 26, 14, 0, 0, 0, time.UTC),
	}

	got := filterEventsForDay([]calendar.Event{timed, otherDay, allDay}, day, dayEnd)
	if len(got) != 2 {
		t.Fatalf("got %d events, want 2: %v", len(got), got)
	}
	// All-day events sort first: they apply to the whole column, so
	// slotting them among the timed events by instant would put them
	// in an arbitrary position.
	if !got[0].AllDay {
		t.Errorf("first event = %q, want the all-day one", got[0].Summary)
	}
	if got[1].Summary != "Standup" {
		t.Errorf("second event = %q, want Standup", got[1].Summary)
	}

	// The previous local day must not pick the all-day event up.
	prev := day.AddDate(0, 0, -1)
	for _, e := range filterEventsForDay([]calendar.Event{allDay}, prev, day) {
		t.Errorf("all-day event leaked into the previous day: %q", e.Summary)
	}
}
