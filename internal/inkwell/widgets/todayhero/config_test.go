...
func TestParseConfig_Defaults(t *testing.T) {
	...
	// Three: the hero agenda spends its height on a scaled time and a
	// wrapped title, so three is what fits before the overflow marker.
...
func TestParseConfig_Rejections(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.label, func(t *testing.T) {
			...
			if !strings.HasPrefix(err.Error(), "today-hero:") {
...
// no today-hero meaning is rejected, with the reason.
