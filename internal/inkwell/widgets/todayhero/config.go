...
// Config defaults. maxEvents is three: the hero agenda spends its
// height on a scaled time and a wrapped title, so three is what fits
// before the overflow marker.
const (
...
// unsupportedKeys are weekly-calendar keys with no today-hero meaning,
