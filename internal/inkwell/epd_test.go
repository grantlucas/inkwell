...
// TestDisplayPartialBoxForcesBoxRedraw locks the core fix: the old plane is the
// previous frame everywhere EXCEPT inside the (byte-aligned) changed box, where
// it is the inverse of the new frame so the controller drives every pixel in the
// box (not just the per-pixel diff). The new plane is the full new frame, and
// the buffers stay full-screen so the capture backends still reconstruct.
func TestDisplayPartialBoxForcesBoxRedraw(t *testing.T) {
	m := &MockHardware{}
	p := partialTestProfile() // 800x480 BW, rowBytes=100
	epd := NewEPD(m, p)

	rowBytes := p.Width / 8
	newBuf := bytes.Repeat([]byte{0xAA}, p.BufferSize())
	prevBuf := bytes.Repeat([]byte{0x00}, p.BufferSize())

	// Box at x=8,y=2,w=16,h=3 → aligned bytes [1,3) on rows 2,3,4.
	if err := epd.DisplayPartialBox(newBuf, prevBuf, Region{X: 8, Y: 2, W: 16, H: 3}); err != nil {
		t.Fatal(err)
	}
	...
	data := m.DataCalls() // VCOM, window, old plane, new plane
	if len(data) != 4 {
		t.Fatalf("data calls = %d, want 4", len(data))
	}
	oldPlane, newPlane := data[2], data[3]

	if len(oldPlane) != p.BufferSize() || len(newPlane) != p.BufferSize() {
		t.Fatalf("planes not full-screen: old=%d new=%d, want %d",
			len(oldPlane), len(newPlane), p.BufferSize())
	}
	if !bytes.Equal(newPlane, newBuf) {
		t.Error("new plane must be the full new frame")
	}
	// Inside the box: forced to ^new (0x55).
	for _, y := range []int{2, 3, 4} {
		for _, b := range []int{1, 2} {
			if got := oldPlane[y*rowBytes+b]; got != 0x55 {
				t.Errorf("oldPlane[row %d byte %d] = %#x, want 0x55 (^new)", y, b, got)
			}
		}
	}
	// Outside the box (different column, same rows; and a row above/below):
	// stays the previous frame (0x00).
	if got := oldPlane[2*rowBytes+0]; got != 0x00 {
		t.Errorf("oldPlane outside box (col 0) = %#x, want 0x00 (prev)", got)
	}
	if got := oldPlane[1*rowBytes+1]; got != 0x00 {
		t.Errorf("oldPlane row above box = %#x, want 0x00 (prev)", got)
	}
	if got := oldPlane[5*rowBytes+1]; got != 0x00 {
		t.Errorf("oldPlane row below box = %#x, want 0x00 (prev)", got)
	}
}

func TestDisplayPartialBoxValidation(t *testing.T) {
	full := func(p *DisplayProfile) []byte { return make([]byte, p.BufferSize()) }
	region := Region{X: 0, Y: 0, W: 16, H: 16}

	gray4Partial := func() *DisplayProfile {
	...
	cases := []struct {
		...
		{"no partial support", smallTestProfile(), make([]byte, 32), make([]byte, 32)},
		{"non-BW color depth", gray4Partial(), full(gray4Partial()), full(gray4Partial())},
		{"new buffer wrong size", partialTestProfile(), []byte{0x00}, full(partialTestProfile())},
		{"prev buffer wrong size", partialTestProfile(), full(partialTestProfile()), []byte{0x00}},
	}
	for _, tc := range cases {
		t.Run(tc.label, func(t *testing.T) {
			err := NewEPD(&MockHardware{}, tc.profile).DisplayPartialBox(tc.newBuf, tc.prevBuf, region)
			if err == nil {
				t.Fatalf("expected error for %s, got nil", tc.label)
			}
		})
	}
}

func TestDisplayPartialBoxSendCommandErrors(t *testing.T) {
	p := partialTestProfile()
	buf := make([]byte, p.BufferSize())
	region := Region{X: 0, Y: 0, W: 16, H: 16}
	...
	for _, n := range []int{1, 2, 3, 4, 5, 6} {
		eh := &errorHardware{failOnCall: n}
		if err := NewEPD(eh, p).DisplayPartialBox(buf, buf, region); err == nil {
			t.Errorf("expected error on SendCommand #%d", n)
		}
	}
}

func TestDisplayPartialBoxSendDataErrors(t *testing.T) {
	p := partialTestProfile()
	buf := make([]byte, p.BufferSize())
	region := Region{X: 0, Y: 0, W: 16, H: 16}

	// 4 SendData calls: VCOM data, window data, old plane, new plane.
	for _, n := range []int{1, 2, 3, 4} {
		ed := &errorDataNthHardware{failOnCall: n}
		if err := NewEPD(ed, p).DisplayPartialBox(buf, buf, region); err == nil {
